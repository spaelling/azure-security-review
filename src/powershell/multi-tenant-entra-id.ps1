...
Note that the tenant needs to have Microsoft Entra ID P2 or Microsoft Entra ID Governance license for some of the checks to succeed
...
$TenantIds = @("1b775964-7849-4f1a-8052-60b8e5c59b96", "690e25b4-8c5e-4a10-a32e-523da88a4c99") # demo tenant, faxmeinthecloud
...
$OutputFolder = "entraid_assessment_$(Get-Date -Format "yyyy_MM_dd_hh_mm")"
# TODO: run commands asynchronously so that we can generate the markdown output much faster - may hit some rate limiting?
foreach ($TenantId in $TenantIds) {
    # paste into browser and consent. the app will appear in the tenant's enterprise applications
    Write-Host "paste this into a browser (once) to consent and add to tenant: https://login.microsoftonline.com/$TenantId/oauth2/authorize?response_type=code&client_id=0614bbae-3283-41e4-b3ac-3946fdcab533&redirect_uri=https://portal.azure.com/"
    ...
    Write-EntraIdAssessment -TenantId $TenantId -AccessToken $AccessToken -OutputFolder $OutputFolder
}
